...
export default function Teams() {
  ...
  return (
    <Section className="container">
      ...
      <div className="team-cards">{query.data?.data.map((team) => <article className="panel team-card" key={team.documentId}>{team.logo ? <img src={getMediaUrl(team.logo.url)} alt={team.name} /> : <div className="team-card__placeholder">{team.name.slice(0, 2).toUpperCase()}</div>}<div><span className="tag">{team.discipline?.name}</span><h2>{team.name}</h2><p>{team.description}</p></div></article>)}</div>
