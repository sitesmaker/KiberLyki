...
export default function OrganizerPanel({ tournament }: { tournament: Tournament }) {
  ...
  const review = useMutation({
    mutationFn: ({ id, status }: { id: string; status: 'approved' | 'rejected' }) => reviewRegistration(id, status),
    onSuccess: refresh,
  });
  ...
  return (
    <section className="panel organizer-panel">
      ...
      <h3>Заявки</h3>
      <div className="members-list">{registrations.data?.data.map((registration) => <div className="member-row" key={registration.documentId}><div><strong>{registration.team?.name}</strong><small>{new Date(registration.submittedAt).toLocaleString('ru-RU')}</small></div><span>{registration.status}</span>{registration.status === 'pending' && <div><button className="mini-action" onClick={() => review.mutate({ id: registration.documentId, status: 'approved' })}>Принять</button><button className="mini-action mini-action--danger" onClick={() => review.mutate({ id: registration.documentId, status: 'rejected' })}>Отклонить</button></div>}</div>)}</div>
      <h3>Результаты</h3>
      <div className="result-list">{tournament.matches?.filter((match) => match.teamA && match.teamB && match.status !== 'finished').map((match) => <form key={match.documentId} onSubmit={(event) => submitResult(event, match.documentId)}><span>{match.teamA?.name}</span><input name="scoreA" type="number" min="0" required /><span>:</span><input name="scoreB" type="number" min="0" required /><span>{match.teamB?.name}</span><Button type="submit" disabled={result.isPending}>Сохранить</Button></form>)}</div>
    </section>
  );
}
